...
export async function saveToBlob() {
  ...
  try {
    ...
    await put('data.json', JSON.stringify(payload), {
      access: 'public',
...
export async function addOrder(data) {
  ...
  const enrichedItems = (data.items || []).map(item => {
    const product = globalThis._bolaginamProducts.find(p => String(p.id) === String(item.productId) || p.name === item.name);
    return {
      ...item,
      costPrice: item.costPrice !== undefined && item.costPrice !== null ? Number(item.costPrice) : (product?.costPrice || 0),
    };
  });
...
export async function deleteOrder(id) {
  ...
  const index = globalThis._bolaginamOrders.findIndex(o => String(o.id) === String(id));
  if (index === -1) return false;
  globalThis._bolaginamOrders.splice(index, 1);
  await saveToBlob();
  return true;
}
...
export function getStats() {
  try {
    ...
    const activeNonCancelledOrders = globalThis._bolaginamOrders.filter(o => o.status !== 'bekor');
    const monthNonCancelledOrders = globalThis._bolaginamOrders.filter(o =>
      o.createdAt && String(o.createdAt).startsWith(thisMonth) && o.status !== 'bekor'
    );

    const productSales = {};
    activeNonCancelledOrders.forEach(order => {
      (order.items || []).forEach(item => {
        ...
        if (!productSales[key]) {
          productSales[key] = { name: item.name || product?.name || '', emoji: item.emoji || product?.emoji || '🎁', total: 0, cost: 0, profit: 0, quantity: 0 };
        }
    ...
    let monthCostSold = 0;
    monthNonCancelledOrders.forEach(order => {
      (order.items || []).forEach(item => {
        if (!item) return;
        const product = globalThis._bolaginamProducts.find(p => String(p.id) === String(item.productId) || p.name === item.name);
        const costPrice = item.costPrice !== undefined && item.costPrice !== null && Number(item.costPrice) > 0 
